...
export default function AllUsers() {
 ...
 const { data: users = [], isLoading, isError } = useQuery({
  ...
  if (isLoading) {
    return <div className="text-center py-20 text-slate-500">Loading Users...</div>;
  }
      ...
      <div className="bg-white rounded-2xl shadow-[0_4px_24px_rgba(99,102,241,0.10)] border border-purple-100 p-6 sm:p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <h1 className="text-2xl font-bold bg-gradient-to-r from-purple-700 via-purple-400 to-purple-500 bg-clip-text text-transparent tracking-tight">All Users</h1>
          </div>
        ...
        <div className="relative mb-4">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-purple-300 text-base">
            🔍
          </span>
          <input
        ...
        {isLoading ? (
          <div className="space-y-3">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="h-16 rounded-xl bg-indigo-50/60 animate-pulse" />
            ))}
          </div>
        ) : filtered.length === 0 ? (
            ...
            {filtered.map((u, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Role Badge */}
                  <span
                    className={`text-[10px] px-3 py-1 rounded-full font-semibold border flex-shrink-0 ${
                      uRole === "provider"
